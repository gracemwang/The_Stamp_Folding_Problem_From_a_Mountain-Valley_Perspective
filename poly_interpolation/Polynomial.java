// Modified from https://gist.github.com/derlin/40545e447fffe7599d26d0a435d9b113

public class Polynomial {
    private Rational[] coef;  // coefficients
    private int deg;     // degree of polynomial (0 for the zero polynomial)

    public Polynomial(int a, int b) {
        this(new Rational(a, 1), b);
    }

    // a * x^b
    public Polynomial(Rational a, int b) {
        coef = new Rational[b + 1];
        for (int i = 0; i < b; i++) {
    ...
    public Polynomial(Polynomial p) {
        coef = new Rational[p.coef.length];
    ...
    public Polynomial plus(Polynomial b) {
        Polynomial a = this;
        Polynomial c = new Polynomial(new Rational(0, 1), Math.max(a.deg, b.deg));
    ...
    public Polynomial times(Polynomial b) {
        Polynomial a = this;
        Polynomial c = new Polynomial(new Rational(0, 1), a.deg + b.deg);
    ...
    public Rational coeff(int degree) {
    ...
    public Polynomial[] divides(Polynomial b) {
        Polynomial q = new Polynomial(new Rational(0, 1), 0);
        Polynomial r = new Polynomial(this);
        while (!r.isZero() && r.degree() >= b.degree()) {
            Rational coef = r.coeff().divide(b.coeff());
    ...
    public Polynomial compose(Polynomial b) {
        Polynomial a = this;
        Polynomial c = new Polynomial(new Rational(0, 1), 0);
    ...
    public boolean isZero() {
        for (Rational i : coef) {
            if (i.getNumerator() == 0) return false;
        }// end for
        return true;
    }
    ...
    public Polynomial differentiate() {
        if (deg == 0) return new Polynomial(new Rational(0, 1), 0);
        Polynomial deriv = new Polynomial(new Rational(0, 1), deg - 1);
        deriv.deg = deg - 1;
        for (int i = 0; i < deg; i++)
            deriv.coef[i] = new Rational(i + 1, 1).multiply(coef[i + 1]);
        return deriv;
    }
    ...
    public String toString() {
        ...
        for (int i = deg - 1; i >= 0; i--) {
            int comp = coef[i].compareTo(new Rational(0, 1));
            if (comp == 0) {
                continue;
            }
            else if (comp > 0) {
            ...
            else if (comp < 0) s = s + " - " + (coef[i].multiply(new Rational(-1, 1)));
